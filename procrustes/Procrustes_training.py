'''
Script for training the Procrustes transformation.
...
def main():
    ...
    # Results directory
    results_dir_root = './Procrustes_results'
    tmp_dir = './tmp'
...
def train_NCconverter(x, y, x_labels, y_labels, idxs_merge_src, idxs_merge_trg, num_sample,
    ...
    for k in idxs_merge_src.keys():
        ...
        source = x_normalized[:,idxs_sub_src]
        target = y_normalized[:,idxs_sub_trg]
        ...
        W, scale = orthogonal_procrustes(source, target)
        if sm < tm:
            W = W[:sm,:]
        elif sm > tm:
            W = W[:, :tm]
