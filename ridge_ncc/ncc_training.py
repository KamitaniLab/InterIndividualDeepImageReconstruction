...
def main():
    ...
    for index, row in df_param.iterrows():
        ...
        analysis_id = analysis_basename + '-' + conversion + '-' + roi + '-' + method +'-' + str(num_samples) + '-' + str(alpha)
        ...
        embedded_idxs_src = compute_roi_index(data_brain[src], rois_list[roi], methods[method])
        embedded_idxs_trg = compute_roi_index(data_brain[trg], rois_list[roi], methods[method])
        ...
        train_NCconverter(x, y,
                          x_labels, y_labels, embedded_idxs_src, embedded_idxs_trg, num_samples,
                          alpha=alpha, 
...
# Functions ##################################################################
def train_NCconverter(x, y, x_labels, y_labels, embedded_idxs_src, embedded_idxs_trg, num_sample,
                      alpha=1.0,
    ...
    for k in embedded_idxs_src.keys():
        ...
        model.fit(x_normalized[:,idxs_sub_src],  y_normalized[:, idxs_sub_trg], alpha=alpha, n_feat=len(idxs_sub_src), chunk_size=0, dtype=np.float32)
        W = model.W
